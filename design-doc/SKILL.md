...
5. Verify `## Decomposition Strategy` is complete and consistent with produced design files:
   ...
   - Run `uv run python scripts/split_check.py <design-file>` before finalizing. If it reports `FAIL`, mark the design as `BLOCKED`; if it reports advisories, tighten `Decision Basis` or boundary ownership before approval.
