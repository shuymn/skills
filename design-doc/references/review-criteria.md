...
- Run `uv run python scripts/split_check.py <design-file>` and use its blocker/advisory output as authoritative signal evidence.
...
**Severity**:
- **Blocker**: Missing `### Boundary Inventory`. `scripts/split_check.py` reports `FAIL`. A root/sub design cannot be reconciled 1:1 between Boundary Inventory and Sub-Doc Index.
- **Warning**: `scripts/split_check.py` reports advisories only.
