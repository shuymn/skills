...
2. **Run Split Checker**: Run `uv run python scripts/split_check.py <design-file>` and keep both blockers and advisories for evidence.
...
4. **Evaluate Each Viewpoint**: For each of the 8 criteria:
   ...
   - For `Decomposition Fit`, use `scripts/split_check.py` output as the authoritative blocker signal.
...
| 8 | Decomposition Fit | PASS/FAIL | [specific evidence, including `scripts/split_check.py` when relevant] |
